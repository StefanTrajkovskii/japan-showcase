...
export default function App() {
  ...
  useLayoutEffect(() => {
    ...
    gsap.set(heroImgRef.current, { scale: isMobile ? 1.05 : 1.15, opacity: 0 })
    gsap.set(navRef.current, { y: -30, opacity: 0 })
    if (kanjiRef.current) gsap.set(kanjiRef.current, { opacity: 0, x: -20 })
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(heroImgRef.current, { scale: isMobile ? 1 : 1.08, opacity: 1, duration: 1.8, ease: 'power2.out' })
      ...
      if (kanjiRef.current) tl.to(kanjiRef.current, { opacity: 1, x: 0, duration: 0.8 }, 1.1)
  ...
  return (
    <div className="overflow-hidden">
      ...
      {/* NAV */}
      <nav ref={navRef} style={{ opacity: 0, transform: 'translateY(-30px)' }} className={`fixed inset-x-0 top-0 z-50 flex items-center justify-between px-4 md:px-[5vw] transition-all duration-500 ${scrolled ? 'py-3 md:py-4 bg-parchment/92 backdrop-blur-[12px] border-b border-ink/8 shadow-[0_6px_26px_rgba(34,27,22,0.06)]' : 'py-4 md:py-[30px] bg-transparent border-b border-transparent'}`}>
        <a href="#top" className={`flex items-baseline gap-2 md:gap-3 no-underline transition-colors duration-400 ${scrolled ? 'text-ink' : 'text-[#fdf7ef]'}`}>
          <span className="font-mincho text-lg md:text-[22px]">物の哀れ</span>
          <span className="text-[9px] md:text-[11px] tracking-[3px] md:tracking-[5px] font-medium opacity-80">AWARE</span>
        </a>
        <div className="flex items-center gap-3 md:gap-[2.6vw]">
          {[['#exhibition','Exhibition'],['#philosophy','Spirit'],['#visit','Visit']].map(([href,label]) => (
            <a key={label} href={href} className={`no-underline text-[13px] tracking-[1px] opacity-90 hidden md:block ${scrolled ? 'text-ink' : 'text-[#fdf7ef]'}`}>
              <TextReveal text={label} />
            </a>
          ))}
          <a href="#visit" className={`text-[11px] md:text-[12.5px] tracking-[1.5px] px-4 md:px-[22px] py-2 md:py-[11px] rounded-full no-underline font-medium transition-all duration-400 ${navBtnCls}`}>Plan your visit</a>
        </div>
      </nav>
      ...
      <section ref={heroRef} id="top" className="relative h-screen min-h-[680px] w-full overflow-hidden">
        <picture>
          ...
          <img ref={heroImgRef} data-gsap-intro src={HERO_MOBILE} alt="Floating torii gate at sunset" className="absolute inset-0 w-full h-full object-cover object-center scale-100 md:scale-[1.08] will-change-transform" />
        </picture>
        ...
        {/* vertical kanji */}
        <div ref={kanjiRef} data-gsap-intro className="hidden md:block absolute top-1/2 left-6 md:left-[4vw] -translate-y-1/2 z-3 font-mincho text-[clamp(15px,1.4vw,19px)] tracking-[10px] text-[#fdf7ef]/78" style={{ writingMode:'vertical-rl', textShadow:'0 2px 14px rgba(40,20,10,0.4)' }}>
          神は水を渡る
        </div>

        {/* hero copy */}
        <div ref={heroCopyRef} data-gsap-intro className="absolute z-3 inset-x-0 top-[15%] md:top-[8%] lg:top-[10%] flex flex-col items-center text-center px-4">
          <p className="text-[9px] md:text-[10px] lg:text-[11px] tracking-[4px] md:tracking-[5px] font-medium text-[#fdf7ef]/70 mb-2 lg:mb-4">日本文化展 · A CULTURAL SHOWCASE</p>
          <h1 className="font-mincho font-medium text-[28px] md:text-[clamp(32px,5vw,58px)] xl:text-[clamp(38px,4.5vw,68px)] text-[#fdf7ef] leading-[1.1] -tracking-[0.5px]" style={{ textShadow:'0 4px 30px rgba(40,18,8,0.35)' }}>Where the gods<br/>cross the water</h1>
        </div>


        {/* scroll indicator */}
        <div ref={scrollIndRef} data-gsap-intro className="absolute inset-x-0 bottom-[26px] z-3 flex flex-col items-center gap-2 text-[#fdf7ef]/70" style={{ animation:'floatNote 2.6s ease-in-out infinite' }}>
